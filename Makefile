...
add-ssh:
	eval `ssh-agent -s`
	ssh-add /home/jonathan/.ssh/azure
...
.PHONY: trivy-test
trivy-test:
	docker build -f docker/Dockerfile_prod -t microblog:$(RELEASE) .
	trivy image microblog:$(RELEASE) --scanners vuln,secret,misconfig --ignorefile .trivy
...
dockle-test:
	docker build -f docker/Dockerfile_prod -t microblog:$(RELEASE) .
	dockle --ignore DKL-LI-003 microblog:$(RELEASE)
